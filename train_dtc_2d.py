    ...
    for epoch_num in iterator:
        ...
        for i_batch,(labeled_sampled_batch, unlabeled_sampled_batch) in enumerate(zip(labeledtrainloader,unlabeledtrainloader)):
            ...
            with torch.no_grad():
                ...
                od_gt_dis = compute_sdf(od_all_label_batch[:].cpu(
                ).numpy(), outputs[:labeled_bs, 0, ...].shape)
                od_gt_dis = torch.from_numpy(od_gt_dis).float().cuda()

                oc_gt_dis = compute_sdf(oc_all_label_batch[:].cpu(
                ).numpy(), outputs[:labeled_bs, 0, ...].shape)
                oc_gt_dis = torch.from_numpy(oc_gt_dis).float().cuda()

            # 计算gt_dis的mse损失, 第2个通道给OD，第三个通道给OC
            loss_sdf = mse_loss(outputs_tanh[:labeled_bs, 1, ...], od_gt_dis) + mse_loss(outputs_tanh[:labeled_bs, 2, ...], oc_gt_dis)

            loss_seg = ce_loss(
                outputs[:labeled_bs, 0, ...], all_label_batch[:labeled_bs].float())
            loss_seg_dice = losses.dice_loss(
                outputs_soft[:labeled_bs, 0, :, :], all_label_batch[:labeled_bs] == 1)
            # 统一将水平集函数转化为mask
            ...
            if iter_num % 50 == 0:
                ...
                writer.add_image('train/DistMap', image, iter_num)

                image = all_label_batch[0]
                writer.add_image('train/Groundtruth_label',
                                 image.unsqueeze(0), iter_num)
