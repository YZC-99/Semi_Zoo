...
from model.module.attentions import SCBAMMModule,SCCBAMMModule


class SCBAM_Module(nn.Module):

    def __init__(self, in_channels_list):
        super(SCBAM_Module, self).__init__()

        self.sa_layers = []
        for idx,in_channels in enumerate(in_channels_list,1):
            sa_layer = 'ska_layer{}'.format(idx)
            sa_layer_module = SCBAMMModule( in_channels)
