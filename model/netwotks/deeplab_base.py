from model.backbone.resnet import resnet34,resnet50, resnet101
...
class BaseNet(nn.Module):
    def __init__(self, backbone):
        ...
        backbone_zoo = {'resnet34': resnet34,'resnet50': resnet50, 'resnet101': resnet101}
        self.encoder = backbone_zoo[backbone](pretrained=True)

    def base_forward(self, x):
        h, w = x.shape[-2:]

        x = self.encoder.base_forward(x)[-1]
        x = self.head(x)
